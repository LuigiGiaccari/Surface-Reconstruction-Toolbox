...
void PCRUST::CopyPoints(double* inputp,int innp)
    {
      ...
      if(Nshield<100)Nshield=100;//Use at least 100 shield points
    ...
    // PQueueNode Node;
     ...
     //add to queue all neighbours of shield tetraedrons
      ...
      for (i=0;i<NT;i++)
         {
         if(deleted[i]){
             if(T[i].T1>=0)AnalyzeIntersection(i,T[i].T1);
             if(T[i].T2>=0)AnalyzeIntersection(i,T[i].T2);
             if(T[i].T3>=0)AnalyzeIntersection(i,T[i].T3);
             if(T[i].T4>=0)AnalyzeIntersection(i,T[i].T4);
             }
...
//    Coord3D tnorm;
        if(T1->T1==T2)
            {p1=T1->p2;p2=T1->p3;p3=T1->p4;p4=T1->p1;}
        else if(T1->T2==T2)
             {p1=T1->p1;p2=T1->p3;p3=T1->p4;p4=T1->p2;}
        else if(T1->T3==T2)
            {p1=T1->p1;p2=T1->p2;p3=T1->p4;p4=T1->p3;}
        else if(T1->T4==T2)
            {p1=T1->p1;p2=T1->p2;p3=T1->p3;p4=T1->p4;}
        else
...
void PCRUST::MarkTetra(int T1,int T2,float Ifact)
    ...
    //after marking analyze the connection with his neighbours
      ...
      if(T1>=0)
       ...
       if(T[T2].T1>=0)AnalyzeIntersection(T2,T[T2].T1);
       if(T[T2].T2>=0)AnalyzeIntersection(T2,T[T2].T2);
       if(T[T2].T3>=0)AnalyzeIntersection(T2,T[T2].T3);
       if(T[T2].T4>=0)AnalyzeIntersection(T2,T[T2].T4);
...
void PCRUST::LookForTriangle(vector<int>* Stack,int T1,int T2)
    {//if THey have different deleted a traignle is generated
     ...
     if (deleted[T1]!=deleted2)//need to generate a triagnle
         ...
         if (deleted[T1]){FindFacet(&T[T2],T1,&facet);}
         else{FindFacet(&T[T1],T2,&facet);}
          ...
          while (!Stack.empty())
              {
                ...
                checked[T1]=true;//T1 is now checked
                ClustId[T1]=idClust;
                 T2=T[T1].T1;if (T2>=0 && !deleted[T2] && !checked[T2]) Stack.push_back(T2);
                 T2=T[T1].T2;if (T2>=0 && !deleted[T2] && !checked[T2]) Stack.push_back(T2);
                 T2=T[T1].T3;if (T2>=0 && !deleted[T2] && !checked[T2]) Stack.push_back(T2);
                 T2=T[T1].T4;if (T2>=0 && !deleted[T2] && !checked[T2]) Stack.push_back(T2);
              }
     ...
     for(i=0;i<NT;i++)
         {
          ...
          c=0;
          if(T[i].T1<0)continue;
          else if (!deleted[T[i].T1])c++;
          if(T[i].T2<0)continue;
          else if (!deleted[T[i].T2])c++;
          if(T[i].T3<0)continue;
          else if (!deleted[T[i].T3])c++;
          if(T[i].T4<0)continue;
          else if (!deleted[T[i].T4])c++;
