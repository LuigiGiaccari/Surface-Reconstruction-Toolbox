...
void STL_FILE_MANAGER::FreeMemory()
    ...
    if (tnorm!=NULL){ delete [] tnorm;tnorm=NULL;}
    if (Header!=NULL){ delete [] Header;Header=NULL;}
    NP=0;
    NT=0;
    Type='n';
    }
...
int STL_FILE_MANAGER::ImportBinary(char* file_name)
    ...
    if (pFile==NULL) {return 3;} //can not open file
...
int STL_FILE_MANAGER::ImportAscii(char* file_name)
    ...
    do{
        ...
        if (FirstChar=='v')//vertex data
            ...
            tnorm[cn].x=x;tnorm[cn].y=y;tnorm[cn].z=z;
            cn++; 
            }
