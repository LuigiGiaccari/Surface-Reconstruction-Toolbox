...
void RCRUST::CopyPoints(vector<double>* inputp,int innp)
    {
      ...
      if(N<10000)Nshield=100;//Use at least 100 shield points
	   if(N<1000)Nshield=50;//Use at least 50 shield points
...
*/
int i;//,flag;
double* u=NULL;Allocate(&u,n);
double* t=NULL;Allocate(&t,n);
double* r=NULL;Allocate(&r,n);
...
//z equals u;
for (i=0;i<n;i++){ps[i].z=u[i];}
//r = √(1−z2);
for (i=0;i<n;i++){r[i]=sqrt(1-u[i]*u[i]);}
// x and y
for (i=0;i<n;i++){
    ps[i].x=r[i]*cos(t[i]);
    ps[i].y=r[i]*sin(t[i]);
    ...
    // PQueueNode Node;
     ...
     //add to queue all neighbours of shield tetraedrons
      ...
      for (i=0;i<NT;i++)
         {
         if(deleted[i]){
             if(Tneigh[i].T1>=0)AnalyzeIntersection(i,Tneigh[i].T1);
             if(Tneigh[i].T2>=0)AnalyzeIntersection(i,Tneigh[i].T2);
             if(Tneigh[i].T3>=0)AnalyzeIntersection(i,Tneigh[i].T3);
             if(Tneigh[i].T4>=0)AnalyzeIntersection(i,Tneigh[i].T4);
             }
...
//stop for debug
      ...
      if (checked[T2])return;//no need to check go forward
...
//    Coord3D tnorm;
        if(Tneigh[T1].T1==T2)
            {p1=T[T1].p2;p2=T[T1].p3;p3=T[T1].p4;p4=T[T1].p1;}
        else if(Tneigh[T1].T2==T2)
		     {p1=T[T1].p1;p2=T[T1].p3;p3=T[T1].p4;p4=T[T1].p2;}
        else if(Tneigh[T1].T3==T2)
		     {p1=T[T1].p1;p2=T[T1].p2;p3=T[T1].p4;p4=T[T1].p3;}
        else if(Tneigh[T1].T4==T2)
		    {p1=T[T1].p1;p2=T[T1].p2;p3=T[T1].p3;p4=T[T1].p4;}
        else
            {Error("Facet error");}
...
void RCRUST::Marking()
   ...
   int c=0;
    while(!queue.empty())
        {
        Node=queue.top();queue.pop();//get and delete the last element form the queue

        if(Node.T2<0 || checked[Node.T2])continue;//no need to check outside tetra our already checked
...
void RCRUST::MarkTetra(int T1,int T2,float Ifact)
    ...
    //after marking analyze the connection with his neighbours
      ...
      if (checked[T2])return;//no need to recheck
      ...
      if(T1>=0)
       ...
       if(Tneigh[T2].T1>=0)AnalyzeIntersection(T2,Tneigh[T2].T1);
       if(Tneigh[T2].T2>=0)AnalyzeIntersection(T2,Tneigh[T2].T2);
       if(Tneigh[T2].T3>=0)AnalyzeIntersection(T2,Tneigh[T2].T3);
       if(Tneigh[T2].T4>=0)AnalyzeIntersection(T2,Tneigh[T2].T4);
...
void RCRUST::GenerateTriangles()
    ...
    int T1;
      ...
      while (!Stack.empty())
          {
          T1=Stack.back();Stack.pop_back();//get and delete element
    ...
    {//if THey have different deleted a traignle is generated
     ...
     bool deleted2;
         ...
         if(T2>=0){
             ...
             deleted2=deleted[T2];
             Stack->push_back(T2);}//add to stack}
         else {if(checked[T1])
             {return;}
             deleted2=true;}//outside tetraedrons are deleted
     ...
     if (deleted[T1]!=deleted2)//need to generate a triagnle
         ...
         if (deleted[T1]){FindFacet(T2,T1,&facet);}
         else{FindFacet(T1,T2,&facet);}
           if(facet.p1<N && facet.p3<N && facet.p3<N )
               {t.push_back(facet);}//new triangle is generated}
           else
...
void RCRUST::Clustering()
    {//set  to not deleted the biggest cluster of tetraedrons
      ...
      int i;
       ...
       int* ClustId=NULL;AllocateAndInit(&ClustId,NT,-1);
     ...
     while(1)
          ...
          while (!Stack.empty())
              {
                ...
                checked[T1]=true;//T1 is now checked
                ClustId[T1]=idClust;
                 T2=Tneigh[T1].T1;if (T2>=0 && !deleted[T2] && !checked[T2]) Stack.push_back(T2);
                 T2=Tneigh[T1].T2;if (T2>=0 && !deleted[T2] && !checked[T2]) Stack.push_back(T2);
                 T2=Tneigh[T1].T3;if (T2>=0 && !deleted[T2] && !checked[T2]) Stack.push_back(T2);
                 T2=Tneigh[T1].T4;if (T2>=0 && !deleted[T2] && !checked[T2]) Stack.push_back(T2);
              }
         ...
         for(i=0;i<NT;i++)
             {
             if(ClustId[i]>=0){NumTetra[ClustId[i]]++;}
             }
         //finding the biggerCLuster
         BiggerClust=0;
         for(i=1;i< idClust;i++)
             {if(NumTetra[i]>NumTetra[BiggerClust])BiggerClust=i;}
...
int RCRUST::FindUnClustered()
    {//finds the first unclustered tetraedron id
     //returns -1 if all points are clustered
      ...
      for(i=0;i<NT;i++)
          {if(!checked[i] && !deleted[i])return i; }
      return -1;
...
void RCRUST::FillWedges()
    ...
    char c;//int8
     for(i=0;i<NT;i++)
         {
          if(!deleted[i])continue;//already filled
          c=0;
          if(Tneigh[i].T1<0)continue;
          else if (!deleted[Tneigh[i].T1])c++;
          if(Tneigh[i].T2<0)continue;
          else if (!deleted[Tneigh[i].T2])c++;
          if(Tneigh[i].T3<0)continue;
          else if (!deleted[Tneigh[i].T3])c++;
          if(Tneigh[i].T4<0)continue;
          else if (!deleted[Tneigh[i].T4])c++;
          if (c==3)deleted[i]=false;//fill wedge
         }
    }
...
void RCRUST::DeleteSlivers()
    ...
    double det;
      for(i=0;i<NT;i++)
         {
          det=TetraVol(i);
          if(det<1e-14 && det>-1e-14)deleted[i]=true;
         }
