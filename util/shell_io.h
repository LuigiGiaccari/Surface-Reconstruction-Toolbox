...
void ReadInputs(int argc,char *argv[])
{
    ...
    for(i=1; i<argc; i=i+2)
    {
        ...
        {
            temp=atoi(argv[i+1]);    //set pause =false if value ==0
            if(temp==0)PAUSE_BEFORE_EXIT=false ;
        }
        ...
        {
            cout<<"Unknow input Type: '"<<argv[i]<<"' for input number: "<< (i+1)/2<<endl;
