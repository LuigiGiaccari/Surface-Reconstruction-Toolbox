...
void FillBuffer(double* p,int N,int *t,int nt)
    {
	  ...
	  {    
		          ...
		          idt=i*3;
                  idb=i*12;
		          P1=t[idt]*3;P2=t[idt+1]*3;P3=t[idt+2]*3;
...
void Tnorm(double* p1,double* p2,double* p3,GLfloat* tnorm)
    {
     ...
     for(i=0;i<3;i++){v21[i]=p2[i]-p1[i];}
     for(i=0;i<3;i++){v31[i]=p3[i]-p1[i];}
    ...
    for(i=0;i<3;i++){tnorm[i]=tnorm[i]/leng;}
 }
...
void InitRenderer()
{
	...
	eyey = BarY;
	eyez = BarZ+Diam*3;
	focusx =BarX; focusy =BarY;  focusz = BarZ;//focus
    CRotX=BarX; CRotY=BarY; CRotZ=BarZ;//center of rotation
...
void * font = GLUT_BITMAP_HELVETICA_12;
	...
	Text = "PAN: mouse R&L";y=y-h;
    DrawText(&Text,font,x,y, r,g,b);
	Text = "ROTATE: mouse L";y=y-h;
    DrawText(&Text,font,x,y, r,g,b);
	Text = "FOCUS: f";y=y-h;
    DrawText(&Text,font,x,y, r,g,b);
	Text = "RESET: r";y=y-h;
